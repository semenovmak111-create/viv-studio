...
def head(w, h, label, extra_defs=""):
    ...
    return (
        ...
        '<stop offset="100%" stop-color="#0E1F18" stop-opacity=".42"/></radialGradient>\n'
        f'{extra_defs}\n'
        '</defs>\n'
    )
...
def hero(w=640, h=980):
    defs = ('  <linearGradient id="bg" x1=".1" y1="0" x2=".8" y2="1">'
            '<stop offset="0" stop-color="#33584A"/><stop offset=".45" stop-color="#1A382E"/>'
            '<stop offset="1" stop-color="#0B1A14"/></linearGradient>\n'
            '  <linearGradient id="win" x1="0" y1="0" x2=".8" y2="1">'
            '<stop offset="0" stop-color="#FDF3D9"/><stop offset="1" stop-color="#E9CF97"/></linearGradient>\n'
            '  <linearGradient id="couch" x1="0" y1="0" x2="0" y2="1">'
            '<stop offset="0" stop-color="#F2ECD9"/><stop offset=".45" stop-color="#D6CDB2"/>'
            '<stop offset="1" stop-color="#8E8B6F"/></linearGradient>')
    s = head(w, h, "Кабинет студии VIV Studio", defs)
    s += f'<rect width="{w}" height="{h}" fill="url(#bg)"/>\n'

    # дальний план — свет
    s += '<g filter="url(#far)">'
    s += el(250, 250, 300, 300, "#F7E7BE", .40)      # разлив света от окна
    s += el(120, 620, 180, 220, "#3E6A58", .30)      # тёплая стена
    s += el(600, 940, 200, 160, "#08140F", .72)      # тёмный угол
    s += '</g>\n'

    # средний план — окно, стена, кушетка
    s += '<g filter="url(#mid)">'
    s += rc(96, 118, 250, 330, "url(#win)", .80, 5)  # окно
    s += rc(0, 648, w, 14, "#2E5748", .55)           # линия стены и пола
    s += rc(-30, 686, w + 60, 250, "url(#couch)", .95, 30)   # кушетка
    s += rc(-30, 686, w + 60, 34, "#FAF4E2", .55, 16)        # блик на валике
    s += rc(452, 470, 92, 200, "#0D1D17", .55, 10)   # стойка аппарата в тени
    s += '</g>\n'

    # ближний план — переплёт окна, экран, полотенца
    s += '<g filter="url(#near)">'
    s += rc(218, 118, 6, 330, "#A98C55", .45)        # переплёт
    s += rc(96, 268, 250, 6, "#A98C55", .45)
    s += rc(468, 500, 60, 38, "#EFE0B4", .72, 3)     # экран аппарата
    s += rc(120, 640, 120, 22, "#F6F0DE", .60, 6)    # стопка полотенец
    s += rc(126, 622, 108, 20, "#EBE4CD", .50, 6)
    s += el(360, 636, 16, 26, "#DFC894", .60)        # флакон на столике
    s += '</g>\n'
    s += '<g filter="url(#near)" opacity=".5">' + el(300, 706, 150, 26, "#FFF7E0", .55) + '</g>\n'
    s += tail(w, h)
    write("hero.svg", s)
...
def master(w=520, h=650):
    defs = ('  <linearGradient id="bg" x1=".2" y1="0" x2=".9" y2="1">'
            '<stop offset="0" stop-color="#F0EADA"/><stop offset=".6" stop-color="#D9D6C0"/>'
            '<stop offset="1" stop-color="#A8AE99"/></linearGradient>')
    s = head(w, h, "Екатерина — мастер VIV Studio", defs)
    s += f'<rect width="{w}" height="{h}" fill="url(#bg)"/>\n'
    s += '<g filter="url(#far)">'
    s += el(110, 90, 210, 180, "#FDF6E4", .85)       # свет слева сверху
    s += el(452, 596, 140, 130, "#6E8577", .34)      # тень справа
    s += '</g>\n'
    s += '<g filter="url(#mid)">'
    s += el(262, 520, 218, 230, "#F5F2EC", .95)      # халат
    s += el(262, 164, 132, 100, "#4A3722", .92)      # волосы
    s += el(262, 260, 112, 136, "#E7BE97", .96)      # лицо
    s += el(166, 276, 42, 84, "#42311E", .78)        # пряди
    s += el(358, 276, 42, 84, "#42311E", .78)
...
def before_after(name, label, before, after, w=520, h=347):
    ...
    defs = ('  <linearGradient id="bgL" x1="0" y1="0" x2="1" y2="1">'
            ...
            '<stop offset="0" stop-color="#0E1F18" stop-opacity="0"/>'
            '<stop offset="1" stop-color="#0E1F18" stop-opacity=".55"/></linearGradient>\n'
...
def map_svg(w=760, h=522):
    s = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" width="{w}" height="{h}" '
         ...
         f'<rect width="{w}" height="{h}" fill="#EAE3D2"/>\n')
    ...
    s += ('<g stroke="#FBF8F0" stroke-linecap="square">'
          ...
          '<g stroke="#D2CBB2" stroke-width="1" stroke-dasharray="7 7">'
          '<path d="M0 212H760"/><path d="M424 0V522"/></g>\n')

    s += ('<g fill="#BFCCB0" opacity=".9"><circle cx="678" cy="150" r="44"/>'
          '<circle cx="702" cy="196" r="28"/><circle cx="122" cy="470" r="30"/></g>\n')

    s += ('<g font-family="Golos, Manrope, sans-serif" fill="#6C5521" font-size="11" '
          ...
          '<text x="606" y="122" fill="#5A7059">САД</text></g>\n')

    s += ('<g font-family="Golos, Manrope, sans-serif" font-size="11.5" font-weight="600">'
          '<circle cx="220" cy="212" r="8" fill="#41473A"/>'
          '<text x="238" y="207" fill="#41473A">М «Чернышевская»</text>'
          '<text x="238" y="223" fill="#585D4E" font-weight="400">12 минут пешком</text>'
          '<circle cx="220" cy="400" r="8" fill="#41473A"/>'
          '<text x="238" y="395" fill="#41473A">М «Площадь Восстания»</text>'
          '<text x="238" y="411" fill="#585D4E" font-weight="400">15 минут пешком</text></g>\n')

    s += ('<g><path d="M424 252c-19 0-34 15-34 34 0 25 34 58 34 58s34-33 34-58c0-19-15-34-34-34z" fill="#1F4A3B"/>'
          '<circle cx="424" cy="286" r="11" fill="#F4EFE3"/>'
          '<g font-family="Cormorant, Georgia, serif" font-size="20" font-weight="600" fill="#23261F">'
          '<text x="470" y="286">VIV Studio</text></g>'
          '<g font-family="Golos, Manrope, sans-serif" font-size="11.5" fill="#41473A">'
...
<defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#F8F4E8"/><stop offset="1" stop-color="#E6E0CE"/></linearGradient></defs>
<rect width="{w}" height="{h}" fill="url(#g)"/>
<rect x="820" width="380" height="{h}" fill="#0F211B"/>
<g font-family="Cormorant, Georgia, serif" fill="#23261F">
  ...
  <text x="76" y="378" font-size="72" font-weight="500" font-style="italic" fill="#1F4A3B">которая заканчивается</text>
</g>
<g font-family="Golos, Manrope, sans-serif" fill="#41473A" font-size="20"><text x="76" y="438">Санкт-Петербург · Суворовский, 35</text></g>
<g font-family="Cormorant, Georgia, serif" fill="#1F4A3B"><text x="76" y="544" font-size="52" font-weight="600">1 450 ₽</text></g>
<g font-family="Golos, Manrope, sans-serif" fill="#585D4E" font-size="17"><text x="222" y="544">первый визит вместо 2 900 ₽</text></g>
<g font-family="Cormorant, Georgia, serif" fill="#DFC894" text-anchor="middle"><text x="1010" y="316" font-size="104" font-weight="500">5,0</text></g>
<g font-family="Golos, Manrope, sans-serif" fill="#ADA894" font-size="15" text-anchor="middle"><text x="1010" y="352">53 оценки на Яндекс Картах</text></g>
</svg>
'''
    write("og.svg", s)
...
if __name__ == "__main__":
    ...
    interior("reception.svg", "Ресепшн студии", ("#F3EEDD", "#DFDAC4", "#ABB09A"),
             far=[el(70, 60, 160, 130, "#FDF6E6", .88), el(400, 300, 140, 110, "#6E8577", .32)],
             mid=[rc(0, 196, 460, 10, "#2E5748", .40),                 # линия стены и пола
                  rc(58, 206, 344, 150, "#1F4A3B", .88, 6),            # стойка — нефрит
                  rc(58, 206, 344, 16, "#E8DFC4", .75, 4),             # столешница в свету
                  rc(292, 62, 96, 106, "#FAF4E4", .82, 3),             # рамка на стене
                  rc(302, 74, 76, 82, "#DDD6BE", .70, 2)],
             near=[el(120, 186, 17, 30, "#DFC894", .85),               # ваза
                   el(360, 168, 22, 22, "#FBF0CE", .85),               # лампа
                   rc(196, 190, 66, 14, "#F4EEDC", .60, 3)])           # стопка бумаг

    # кабинет: окно, кушетка, аппарат
    interior("room.svg", "Кабинет студии", ("#F4EFDF", "#DCD9C1", "#9EA890"),
             far=[el(360, 70, 170, 140, "#FEF9EC", .9), el(60, 300, 120, 100, "#5E7E6E", .34)],
             mid=[rc(0, 190, 460, 10, "#2E5748", .38),
                  rc(300, 34, 132, 150, "#FCF2DA", .88, 3),            # окно
                  rc(46, 214, 330, 92, "#F3EEDC", .94, 22),            # кушетка
                  rc(46, 214, 330, 18, "#FCF8EC", .80, 10),
                  rc(392, 176, 52, 130, "#1F4A3B", .68, 6)],           # аппарат у стены
             near=[rc(364, 34, 5, 150, "#A98C55", .50),                # переплёт окна
                   rc(300, 106, 132, 5, "#A98C55", .50),
                   el(104, 206, 30, 12, "#FCF8EC", .6)])               # подушка

    # аппарат: корпус, экран, манипулятор
    interior("device.svg", "Лазерный аппарат", ("#EFE9D6", "#D2CDB5", "#95A08C"),
             far=[el(378, 84, 120, 110, "#FBF1D6", .70), el(90, 300, 130, 100, "#5E7E6E", .32)],
             mid=[rc(0, 236, 460, 10, "#2E5748", .36),
                  rc(158, 96, 148, 210, "#12251E", .92, 10),           # корпус
                  rc(176, 118, 112, 66, "#EDE3BC", .88, 4),            # экран
                  rc(176, 200, 112, 10, "#2E5748", .70, 4)],
             near=[rc(292, 130, 96, 8, "#1B3A2F", .75, 4),             # манипулятор
                   el(392, 140, 18, 14, "#0C1913", .8),
                   el(232, 152, 26, 16, "#FBF4D8", .35)])              # блик на экране

    # сертификаты: рамки на стене и полка
    interior("certs.svg", "Сертификаты мастера", ("#F4EFDE", "#DFDAC2", "#ADB299"),
             far=[el(60, 60, 140, 120, "#FEFAEC", .75), el(400, 290, 130, 100, "#6E8577", .32)],
             mid=[rc(74, 66, 118, 152, "#FBF6E6", .95, 3),
                  rc(88, 82, 90, 120, "#DFD8BE", .80, 2),
                  rc(216, 92, 104, 132, "#FBF6E6", .92, 3),
                  rc(228, 106, 80, 104, "#DFD8BE", .78, 2),
                  rc(344, 70, 82, 106, "#FBF6E6", .88, 3),
                  rc(354, 82, 62, 82, "#DFD8BE", .74, 2),
                  rc(0, 268, 460, 12, "#2E5748", .40)],
             near=[el(80, 258, 18, 26, "#DFC894", .75)])
